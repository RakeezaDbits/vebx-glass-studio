            ...
            <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="liquid-glass rounded-2xl p-8 border-glow">
              ...
              <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
                <div className="grid sm:grid-cols-2 gap-4">
                  <Input placeholder={t("contact.yourName")} className="bg-secondary/50 border-border" />
                  <Input placeholder={t("contact.yourEmail")} type="email" className="bg-secondary/50 border-border" />
                </div>
                <Input placeholder={t("contact.subject")} className="bg-secondary/50 border-border" />
                <Textarea placeholder={t("contact.tellUs")} rows={5} className="bg-secondary/50 border-border resize-none" />
                <Button variant="hero" size="lg" className="w-full gap-2">{t("contact.sendBtn")} <Send className="w-4 h-4" /></Button>
              </form>
            </motion.div>
