            ...
            {techStacks.map((stack, i) => (
              <motion.div key={stack.categoryKey} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: i * 0.1 }} className="liquid-glass rounded-2xl p-8 border-glow">
                ...
                <div className="flex flex-wrap gap-2">
                  {stack.techs.map((tech) => (<span key={tech} className="px-3 py-1.5 rounded-full text-xs font-medium bg-secondary text-muted-foreground hover:bg-primary/20 hover:text-foreground transition-colors cursor-default">{tech}</span>))}
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <div className="container px-4 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
            {industryKeys.map((key, i) => (
              <motion.div key={key} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.08 }} className="liquid-glass rounded-xl p-5 border-glow text-center hover:bg-primary/5 transition-all duration-500">
